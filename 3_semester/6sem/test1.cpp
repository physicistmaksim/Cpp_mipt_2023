...
class String{
private:
    size_t m_size = 0;    char *m_storage = nullptr;
    ...
    {        for (size_t i = 0; i < m_size; ++i)
            m_storage[i] = el;    }
public:
    // Конструктор    explicit String(size_t size = 0, char el = 'a') : m_size(size), m_storage(new char[m_size])
    {        assign_element(el);
    }
    // Копирующий конструктор    String(const String &other) : m_size(other.m_size), m_storage(new char[m_size])
    {        for (size_t i = 0; i < m_size; ++i)
    ...
    // Оператор перемещающего присваивания    String &operator=(String &&other) noexcept
    {        if (this != &other)
            swap(other);        return *this;
    }
    // Оператор +=    String &operator+=(const String &other)
    {        auto new_size = m_size + other.m_size;
        auto *new_storage = new char[new_size];
        for (size_t i = 0; i < m_size; ++i)            new_storage[i] = m_storage[i];
        for (size_t i = 0; i < other.m_size; ++i)
            new_storage[m_size + i] = other.m_storage[i];
        delete[] m_storage;        m_size = new_size;
        m_storage = new_storage;
        return *this;    }
    // Внешний оператор сложения
    friend String operator+(String const &lha, String const &rha)    {
        String res(lha);        return res += rha;
    }
    // Возвращает размер строки    [[nodiscard]] size_t size() const
    {        return m_size;
    }
    // Возвращает элемент строки с индексом idx    char &operator[](size_t idx)
    {        return m_storage[idx];
    }
    // Операторы ввода-вывода    friend std::ostream &operator<<(std::ostream &out, const String &obj)
